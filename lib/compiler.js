...
exports.compile = function(ast, modules, declarations) {
	...
	return code(
		...
		'})(); // let\'s kick it',
		{
			...
			code: exports.compileRaw(ast, rootHook),
...
var compileExpression = function(context, ast) {
	switch(ast.type) {
		...
		default:                  log(ast.type, ast); UNKNOWN_EXPRESSION_TYPE
	}
}
...
var compileStatement = function(context, ast) {
	switch (ast.type) {
		...
		default:                  log(ast.type, ast); UNKNOWN_STATEMENT_AST_TYPE
	}
}
...
var _runtimeValue = function(ast) {
	...
	switch(ast.type) {
		...
		default:                  log(ast); UNKNOWN_RUNTIME_VALUE_AST_TYPE
	}
}
...
var getPropertyName = function(ast) {
	switch(ast.type) {
		...
		default:
			log(ast); UNKNOWN_PROPERTY_AST_TYPE
	}
}
...
var compileDeclaration = function(declaration) {
	switch (declaration.type) {
		...
		default:              util.halt(declaration, 'Found declaration that requires compilation of unknown type')
	}
}
...
var _compileMutationStatement = function(ast) {
	...
	switch(ast.value.type) {
		...
		default:
			log(ast.value); UNKNOWN_MUTATION_VALUE_AST_TYPE
	}
}
...
var compileJavascriptBridge = function(ast) {
	...
	switch (value.jsType) {
		...
		default:         log(ast); UNKNOWN_AST_JS_TYPE
	}
}
...
var _collectDynamicASTs = function(ast) {
	...
	switch(ast.type) {
		...
		default:
			log(ast); UNKNOWN_DYNAMIC_AST_TYPE
	}
}
...
function _compileStatementValue(ast) {
	...
	switch(ast.type) {
		...
		default:
			log(ast); UNKNOWN_VALUE_AST_TYPE
	}
}
...
var assert = function(ast, ok, msg) { if (!ok) util.halt(ast, msg) }
